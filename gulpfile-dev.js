...
gulp.task('jekyll-build', ['sass', 'js'], function (done) {
    ...
    return cp.spawn(pl, ['build', '--incremental', 'JEKYLL_ENV=development'], {stdio: 'inherit'})
        .on('close', done);
});
 ...
 */
gulp.task('browser-sync', ['sass', 'js', 'foundation-js', 'jquery-js', 'imgmv', 'jekyll-build'], function() {
    browserSync({
        server: {
            baseDir: '_site'
        },
...
gulp.task('watch', function () {
    gulp.watch('_assets/img/**', ['imgmv']).on("change", browserSync.reload);;
    ...
    gulp.watch(['_assets/js/**/*.js','_assets/img/**','_assets/css/**','*.html', '_layouts/*.html', '_posts/*', '_includes/*'], ['jekyll-rebuild']);
});
 ...
 * Build the Jekyll Site
 */
gulp.task('jekyll-build-prod', ['sass-prod', 'js-prod'], function (done) {
    browserSync.notify(messages.jekyllBuild);
    var pl = process.platform === "win32" ? "jekyll.bat" : "jekyll";
    return cp.spawn(pl, ['build', 'JEKYLL_ENV=production'], {stdio: 'inherit'})
        .on('close', done);
});
...
** JS Task
*/
gulp.task('js-prod', ['sass-prod'], function() {
...
// JSHint, concat, and minify Foundation JavaScript
gulp.task('foundation-js-prod', function() {
  return gulp.src([
...
// JSHint, concat, and minify Jquery JavaScript
gulp.task('jquery-js-prod', function() {
  return gulp.src('_assets/js/jquery/jquery.js')
    //.pipe(jshint())
    //.pipe(.reporter('default'))
    .pipe(gulp.dest('assets/js'))
...
gulp.task('production', ['sass-prod', 'js-prod', 'imgmin', 'foundation-js-prod', 'jquery-js-prod', 'jekyll-build-prod']);
